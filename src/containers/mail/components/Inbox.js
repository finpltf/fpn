...
export default class Inbox extends PureComponent {
  ...
  constructor(props) {
    ...
    this.onMailBox = this.onMailBox.bind(this);
    this.onLetter = this.onLetter.bind(this);
    this.onOpenMailboxes = this.onOpenMailboxes.bind(this);
  }
  ...
  render() {
    return (
      <div className={`inbox${this.state.openMailboxes ? ' inbox--show-mailboxes' : ''}`}>
        <div className='inbox__mailbox-list'>
          <Button color='primary' className='icon inbox__mailbox-list-btn' size='sm' onClick={this.onCompose}><PenIcon/>Compose</Button>
          {mailboxes.map((m, i) =>
            <a key={i} onClick={(e) => this.onMailBox(i, e)}>
              <MailBox title={m.title} amount={m.amount} selected={i === this.state.mailbox}>
                {m.icon}
              </MailBox>
            </a>
          )}
          ...
          {labels.map((l, i) =>
            <a key={i} onClick={(e) => this.onLabel(i, e)}
               className={`inbox__label${this.state.label === i ? ' active' : ''}`}>
              <span className={`inbox__label-color inbox__label-color--${l.color}`}/>{l.title}
            </a>
          )}
        </div>
        <div className='inbox__container'>
          <div className={`inbox__topbar${this.state.email ? ' inbox__topbar--hide' : ''}`}>
            ...
            <div>
              {this.state.email ?
                <Email email={emailExample[0]} onReply={this.onCompose} onSubmit
                       onBack={(e) => this.onMailBox(this.state.mailbox, e)}/>
                :
